...
from library.book_library import year_read_list
...
class YearReadTable(GenericTable):
	...
	def get_selected_year(self):
		...
		if index:
			return year_read_list[index[0]]['year']
	...
	def sort_table(self, mode):
		...
		if mode == '0r': year_read_list.sort(key = lambda x: x['year'], reverse=True)
		elif mode == '0': year_read_list.sort(key = lambda x: x['year'])	
		elif mode == '1r': year_read_list.sort(key = lambda x: x['book_count'])
		elif mode == '1': year_read_list.sort(key = lambda x: x['book_count'], reverse=True)
		elif mode == '2r': year_read_list.sort(key = lambda x: float(x['average_rating']), reverse=True)
		elif mode == '2': year_read_list.sort(key = lambda x: float(x['average_rating']))
		elif mode == '3r': year_read_list.sort(key = lambda x: int(x['total_pages']))
		elif mode == '3': year_read_list.sort(key = lambda x: int(x['total_pages']), reverse=True)
		elif mode == '4r': year_read_list.sort(key = lambda x: float(x['average_length']))
		elif mode == '4': year_read_list.sort(key = lambda x: float(x['average_length']), reverse=True)
