...
from control.library_indexer import get_index
...
def add_or_edit_book(index, title, author, num_pages, publisher, rating, date_read, isbn_10, isbn_13, date_added,
	...
	be overwritten by a new Book object containing both the changed and the unchanged data.
	"""
	if index == None:
		index = get_index()
	...
	books[index] = Book(
		...
		isbn_10,
		isbn_13,
		date_added if date_added is not None else datetime.now(),
		date_started,
		reading_status,
		book_format,
		review,
...
def edit_text_attribute(index, attribute, text):
	...
	setattr(books[index], attribute, text)
