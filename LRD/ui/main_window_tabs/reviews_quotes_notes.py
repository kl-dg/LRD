...
from data.index_lists import books_with_reviews, books_with_quotes, books_with_notes
from control.queries import get_books_with_text
from ui.main_window_proxy import main_window
from ui.misc.cb_constructors import SortReviewsDropDown
...
class TextPageTab(QWidget):
	...
	def __init__(self, attribute):
		...
		self.attribute_lists = {"review": books_with_reviews, "quotes": books_with_quotes, "notes": books_with_notes}
	...
	def sort_tab(self):
		...
		if self.sort_cb.currentIndex() == 0: 
			self.text_box_list.sort(key = lambda x: books[x].title.lower())
		elif self.sort_cb.currentIndex() == 1:
			self.text_box_list.sort(key = lambda x: books[x].author_sorted().lower())
		elif self.sort_cb.currentIndex() == 2:
			self.text_box_list.sort(key = lambda x: books[x].rating, reverse=True)
		elif self.sort_cb.currentIndex() == 3:
			self.text_box_list.sort(key = lambda x: books[x].date_sortable('date_read'), reverse=True)
		elif self.sort_cb.currentIndex() == 4:
			self.text_box_list.sort(key = lambda x: get_int(books[x].num_pages), reverse=True)
		elif self.sort_cb.currentIndex() == 5:
			self.text_box_list.sort(key = lambda x: get_int(books[x].original_publication_year), reverse=True)
