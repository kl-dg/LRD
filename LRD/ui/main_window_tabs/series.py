...
from data.index_lists import books_by_selected_series_or_collection
from control.queries import get_books_by_series_or_collection
...
from ui.tables.series_table import SeriesTable
...
class SeriesTab(QWidget):
	...
	def __init__(self):
		...
		self.books_by_series_table = BookTable(self, books_by_selected_series_or_collection)
	...
	def clicked_edit_attribute(self, table, attribute):
		...
		args:
		should be series_table and 'series' or 
		collection_table and 'collection'.
		"""
	...
	def refresh_books_by_series_table(self):
		...
		if self.current_table == 'series':
			...
			books_by_selected_series_or_collection.sort(key = lambda x: books[x].volume_in_series)
			
		elif self.current_table == 'collection':
			get_books_by_series_or_collection(self.collection_table.selected_item, 'collection')
			books_by_selected_series_or_collection.sort(key = lambda x: books[x].volume_in_collection)
