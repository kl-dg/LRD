...
from data.libraries import authors
from control.queries import get_list_of_authors
...
class AuthorTable(GenericTable):
	...
	def sort_table(self, mode):
		...
		if mode == '0r': authors.sort(key = lambda x: f"{x['author'].lower().split()[-1]}, {' '.join(x['author'].lower().split()[0:-1])}" if ',' not in x['author'] else x['author'].lower(), reverse=True)
		elif mode == '0': authors.sort(key = lambda x: f"{x['author'].lower().split()[-1]}, {' '.join(x['author'].lower().split()[0:-1])}" if ',' not in x['author'] else x['author'].lower())
		elif mode == '1r': authors.sort(key = lambda x: x['book_count'])
		elif mode == '1': authors.sort(key = lambda x: x['book_count'], reverse=True)
		elif mode == '2r': authors.sort(key = lambda x: float(x['average_rating']))
		elif mode == '2': authors.sort(key = lambda x: float(x['average_rating']), reverse=True)
		elif mode == '3r': authors.sort(key = lambda x: float(x['average_length']))
		elif mode == '3': authors.sort(key = lambda x: float(x['average_length']), reverse=True)
