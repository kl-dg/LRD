...
from data.libraries import bookshelves
from control.queries import get_bookshelves_list
...
class BookshelvesTable(GenericTable):
	...
	def get_selected_bookshelf(self):
		...
		if index:
			return bookshelves[index[0]]['bookshelf']
	...
	def sort_table(self, mode):
		...
		if mode == '0r': bookshelves.sort(key = lambda x: x['bookshelf'].lower(), reverse=True)
		elif mode == '0': bookshelves.sort(key = lambda x: x['bookshelf'].lower())	
		elif mode == '1r': bookshelves.sort(key = lambda x: x['book_count'])
		elif mode == '1': bookshelves.sort(key = lambda x: x['book_count'], reverse=True)
		elif mode == '2r': bookshelves.sort(key = lambda x: float(x['average_rating']), reverse=True)
		elif mode == '2': bookshelves.sort(key = lambda x: float(x['average_rating']))
		elif mode == '3r': bookshelves.sort(key = lambda x: float(x['average_length']))
		elif mode == '3': bookshelves.sort(key = lambda x: float(x['average_length']), reverse=True)
