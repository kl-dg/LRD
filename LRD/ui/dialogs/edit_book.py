...
from functions.date_formatting import ddmmyyyy_to_datetime
...
from control.edit_library import add_or_edit_book
from assets.strings import available_formats, reading_statuses
from ui.main_window_proxy import main_window
from ui.misc.cb_constructors import (
	DayDropDown, 
	FormatDropDown, 
	MonthDropDown, 
	RatingDropDown,
...
class EditBook(QWidget):
	...
	def information_tab_settings(self):
		...
		if self.index is not None: date_added_label = QLabel(f"Date added: {books[self.index].get_date_as_string('date_added', '%d/%b/%Y %H:%M')}.")
	...
	def preview_current_info(self):
		...
		self.title_field.setText(books[self.index].title)
		self.author_field.setText("; ".join(books[self.index].author))
		self.translator_field.setText(books[self.index].translator)
		self.pages_field.setText(books[self.index].num_pages)
		self.isbn10_field.setText(books[self.index].isbn10)
		self.isbn13_field.setText(books[self.index].isbn13)
		self.format_cb.setCurrentIndex(self.format_cb.findText(self.to_format_cb(books[self.index].book_format)))
		self.publication_year_field.setText(books[self.index].edition_publication_year)
		self.publisher_field.setText(books[self.index].publisher)
		self.series_field.setText(books[self.index].series)
		self.volume_in_series_field.setText(books[self.index].volume_in_series)
		self.collection_field.setText(books[self.index].collection)
		self.volume_in_collection_field.setText(books[self.index].volume_in_collection)
		self.original_title_field.setText(books[self.index].original_title)
		self.original_publication_year_field.setText(books[self.index].original_publication_year)
		self.number_of_volumes_sb.setValue(self.set_number_of_volumes(books[self.index].number_of_volumes))
		self.bookshelves_field.setText("; ".join(books[self.index].bookshelves))
		self.day_bought.setCurrentIndex(books[self.index].date_bought.day if books[self.index].date_bought else 0)
		self.month_bought.setCurrentIndex(books[self.index].date_bought.month if books[self.index].date_bought else 0)
		self.year_bought.setCurrentIndex(self.year_bought.findText(str(books[self.index].date_bought.year)) if books[self.index].date_bought else 0)
		self.condition_field.setText(books[self.index].condition)
		self.weblink_field.setText(books[self.index].weblink)
		self.day_started.setCurrentIndex(books[self.index].date_started.day if books[self.index].date_started else 0)
		self.month_started.setCurrentIndex(books[self.index].date_started.month if books[self.index].date_started else 0)
		self.year_started.setCurrentIndex(self.year_started.findText(str(books[self.index].date_started.year)) if books[self.index].date_started else 0)
		self.day_read.setCurrentIndex(books[self.index].date_read.day if books[self.index].date_read else 0)
		self.month_read.setCurrentIndex(books[self.index].date_read.month if books[self.index].date_read else 0)
		self.year_read.setCurrentIndex(self.year_read.findText(str(books[self.index].date_read.year)) if books[self.index].date_read else 0)
		self.reading_status_cb.setCurrentIndex(self.reading_status_cb.findText(self.to_reading_status_cb(books[self.index].reading_status)))
		self.rating_cb.setCurrentIndex(self.rating_cb.findText(to_rating_cb(books[self.index].rating)))
		self.times_read_sb.setValue(self.set_times_read_sb(books[self.index].times_read))
		self.review_box.setText(books[self.index].review)
		self.quote_box.setText(books[self.index].quotes)
		self.notes_box.setText(books[self.index].notes)
	...
	def set_started_today(self):
		...
		self.day_started.setCurrentIndex(datetime.now().day)
		self.month_started.setCurrentIndex(datetime.now().month)
		self.year_started.setCurrentIndex(self.year_started.findText(str(datetime.now().year)))
	...
	def set_read_today(self):
		...
		self.day_read.setCurrentIndex(datetime.now().day)
		self.month_read.setCurrentIndex(datetime.now().month)
		self.year_read.setCurrentIndex(self.year_read.findText(str(datetime.now().year)))
	...
	def edit_book(self):
		...
		add_or_edit_book(
			...
			books[self.index].date_added if self.index is not None else None,
